...
#ifndef SMOOTHINGREGULARIZATION_H
#define SMOOTHINGREGULARIZATION_H
...
namespace PatternOptimization {
    namespace ObjectiveTerms {
        ...
        template<class _Sim>
        struct SmoothingRegularizationTerm : public ObjectiveTerm<_Sim::N> {
            ...
            SmoothingRegularizationTerm(_Iterate &it) : m_sim(it.simulator()) {
            ...
            virtual ~SmoothingRegularizationTerm() { }
        private:
            _Sim &m_sim;
        ...
        template<class _Sim>
        struct IFConfigSmoothingRegularization: public IFConfig {
            template<class _Iterate>
            void configIterate(const std::unique_ptr<_Iterate> &it, ObjectiveTermNormalizations &normalizations) const {
                auto sr = Future::make_unique<SmoothingRegularizationTerm<_Sim>>(*it);
                sr->setWeight(weight);

                if (!normalizations.isSet("SmoothingRegularization"))
                    normalizations.set("SmoothingRegularization", 1.0);

                sr->setNormalization(normalizations["SmoothingRegularization"]);
                it->addObjectiveTerm("SmoothingRegularization", std::move(sr));
            }
            Real weight = 0.0;
        };
...
#endif //SMOOTHINGREGULARIZATION_H